...
# image 1
plotRGB(im1_orig,r=4,g=3,b=2,main="Image")                    # we plot bands (4,3,2) (NIR-R-G) as (R,G,B)
...
calcLocalAverage <- function(img,KS) {
  ...
  for (i in 1:nlayers(img)){
    print(i)
    lyr <-  focal(img[[i]], w=matrix(1,nrow=KS,ncol=KS), fun=mean)
    
    out <- addLayer(out, lyr)
  }
...
calcLocalStandardDeviation <- function(img, KS) {
  ...
  for (i in 1:5){
    var_x = N/(N-1) * ((calcLocalAverage(img[[i]]^2, KS) - calcLocalAverage(img[[i]], KS)^2))
    s_dev <- N/(N-1) * sqrt(((calcLocalAverage(img[[i]]^2, KS) - calcLocalAverage(img[[i]], KS)^2)))
    
    localSd <- addLayer(localSd, s_dev)
  }
...
im1_norm <- normalise(im1, means=NULL, stdDevs=NULL)
im2_norm <- normalise(im2, means=im1_norm[[2]], im1_norm[[3]])
...
for(nT in 1:length(numTrees_range)) {
  for(mL in 1:length(minLeaf_range)) {
    ...
    rf_model <- randomForest(x=x_train[valid_train,], y=y_train[valid_train], ntree=numTrees_range[nT], nodesize=minLeaf_range[mL])
    ##############################
...
rf_model <- randomForest(x=x_train[valid_train,], y=y_train[valid_train], ntree=numTrees_range[nT], nodesize=minLeaf_range[mL], importance=TRUE)
##############################
...
# we decided to keep all variables for the classification. Please see project report for explanation
...
####### YOUR CODE HERE #######
rf_model <- randomForest(x=x_train[valid_train_full,], y=y_train[valid_train_full], ntree=numTrees, nodesize=minLeafSize, importance=TRUE)
##############################
